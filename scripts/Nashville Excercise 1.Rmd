...
As demonstrated by [Statbomb's (via FBRef)](https://fbref.com/en/matches/edd92a6d/Nashville-SC-Orlando-City-November-23-2021-Major-League-Soccer) and [FootyStat's](https://footystats.org/usa/orlando-city-sc-vs-nashville-sc-mls-h2h-stats#1448602) assessment of the match, it is clear that Nashville scarcely deserved to win the game:
...
All in all, __Nashville consistently absorbed their opponent's pressure to create lethal counter-attacking opportunities, but struggled to convert the high-quality chances they produced into dangerous shots__.
...
In 84% of their games over the last two MLS seasons, Philadelphia played in a __4-4-2◆__ formation. __This is the formation we will assume Union will be using in their next match against Nashville SC__. Though it would have been useful to analyze the Union's __4-3-2-1__ formation, which they used in Nashville's latest defeat, they have only ever deployed it a handful of times. 


Interestingly, Nashville SC has only ever played Union three times. When playing with two strikers up front, Nashville got just one point from two games. When playing with only one striker against Philadelphia, Nashville won the game comfortably. __So, it would appear the team should favor playing with a single striker rather than a partnership__, but obviously the sample sizes behind that analysis are ridiculously low to trust its conclusion.


To get more robust results, I analyzed a custom data set that I assembled from [FBRef](https://fbref.com/en/squads/46024eeb/Philadelphia-Union-Stats) (a sample of which is shown below) and performed an [Ordinary Least Squares Regression with Robust Standard Errors](https://www.rdocumentation.org/packages/estimatr/versions/0.30.4/topics/lm_robust) to evaluate the effect that a team's choice of formation against Union's 4-4-2◆ had on their chances of success.
...
To find insights on the individual-level for Nashville's next match against Philadelphia Union, I used [American Soccer Analysis' (ASA) Goals Added](https://www.americansocceranalysis.com/what-are-goals-added) metric to identify the most integral players to Union's play-making and goal scoring. 

The following six players were Philadelphia's top producers in each category. The "Percentile" column tells us what percentage of players scored lower in ASA's rankings during the 2021 MLS season. For example, __Jack Elliott's 0.06 Passing Goals Added/90 was better than 83% of other scores in the same category__.
...
union_passing_goals_added <- union_passing_goals_added[order(union_passing_goals_added$Percentile, decreasing = TRUE), ]
...
To get a general idea of Philadelphia's behavior during set-pieces, I used [ASA's xGoal](https://app.americansocceranalysis.com/#!/mls/xgoals/teams) data to analyze each team's non-corner set-piece and corner kick performances __in terms of efficiency and frequency__ in relation to other MLS teams. Those results shown below:
...
In terms of non-corner set-pieces, Union performed very poorly. The team ranked 24^th^ of 27^th^ in xG per shot during the 2021 MLS season. If they have not improved their approach since last season, they will pose very little threat in 2022.

Additionally, Philadelphia seldom chooses to shoot the ball. In fact, __Union shoots from set-pieces 25% less than LAFC, who led the league in shots from free-kicks__. Instead, __Philadelphia generally chooses to use their free kicks as opportunities to set up subsequent chances that might prove to be more valuable__.

Ironically, __Union were the most productive scorers from corner kicks during the 2021 MLS season__; producing 29% more xG per attempt than the league average. 

#### Takeaways

* Nashville should focus on defending against dangerous passes rather than blocking shots while defending Philadelphia's non-corner set-pieces. If Nashville is able to reliably regain possession from Union, the team should set up to take advantage of potential counter attacking opportunities.
